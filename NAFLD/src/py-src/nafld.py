...
def update_filter_slide(change):
...
def update_filter_image(change):
    ...
    original_image_path = os.path.join(image_folder, image_dropdown.value)
    print(f'\n {original_image_path} \n')
...
def process_images_in_folder(folder_path, predict_cluster_func):
    ...
    df = pd.DataFrame(results, columns=['image_name', 'percentage', 'cluster_label', 'u1', 'u2', 'u3', 'u4'])
    return df


def process_given_file(file_path):

    if(not os.path.isfile(file_path)):
        return f"file path is a folder not a file: {file_path}"

    if file_path.lower().endswith('.zip'):
        with ZipFile(file_path, 'r') as zObject: 
            zObject.extractall(path=os.path.dirname(file_path))
        
    elif file_path.lower().endswith('.png','.jpg','.jpeg','.tiff'):
        pass
    elif file_path.lower().endswith('.svs'):
        pass
    else:
